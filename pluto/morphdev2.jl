...
menu = ["" => "", 
	joinpath(dirname(pwd()), "corpus", "compnov.cex") => "Complutensis texts",
...
tcorpus = isnothing(textselection) ? nothing : tokenizedcorpus(textselection, ortho, filterby = LexicalToken())

# ╔═╡ 518caceb-d790-4d6b-9678-2197b0d4cbbd
# ╠═╡ show_logs = false
analyzedlexical = if isnothing(textselection)
	nothing 
elseif doparse
	parsecorpus(tcorpus, parser)
	#parsecorpus(CitableTextCorpus(textselection),  parser))
else
	nothing
end
...
# ╔═╡ c20af41c-4f07-4dcb-8349-c0bc2fa4ba78
isnothing(sept) ? nothing : filter(sept.passages) do psg
